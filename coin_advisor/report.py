...
def render_report(ranked: List[RankedCoin], generated_at: datetime, top_n: int = 5) -> Tuple[str, str]:
    ...
    plain_lines = [f"Günlük Coin Adayları — {date_text}", "", "Bu bir yatırım tavsiyesi değildir.", ""]
    cards = []
    market_warning = ""
    ...
    </style></head><body><main><div class="header"><h1>Günün En Güçlü {top_n} Coin Adayı</h1><div class="muted">{escape(date_text)}</div><p>Piyasa, göreli güç, likidite ve mevcut DeFi verilerine göre açıklanabilir sıralama.</p></div>
